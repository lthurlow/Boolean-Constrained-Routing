...
__all__ = ['modified_dijkstra', # Lincoln
           'dijkstra_path',
...
## Modified Dijkstra will return all satisfiable paths, in sorted order
## Returning a list of all paths with shortest first, and longest last.
def modified_dijkstra(G, source, target, weight='weight'):
  ## X should be a list of all routes from S to T, each element in the list
  ## should be ([path], cost)
    X=set_source_dijkstra(G, source, target, weight)
    try:
        return X
    except KeyError:
